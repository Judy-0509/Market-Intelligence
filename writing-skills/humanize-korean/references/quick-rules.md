# Quick Rules — Fast/단일 패스 전용 (v2.0, vendored)

> **벤더링 수정 고지:** 원본에서는 이 룰북을 `humanize-monolith` 에이전트가 하위 호출로
> 사용했다. 이 벤더링본에는 별도 에이전트 정의가 포함되지 않으므로, 이 스킬을 실행하는
> 모델이 하위 에이전트 호출 없이 직접 이 룰북을 읽고 적용한다. 규칙 내용 자체는 수정하지
> 않았다.

탐지·윤문·자체검증을 한 번에 끝내기 위해 사용하는 슬림 룰북. 본진 분류 체계(원본
저장소의 `ai-tell-taxonomy.md`, 590줄, 이 벤더링본에는 미포함)에서 S1·S2 핵심 패턴만
추려 처방과 함께 한 줄로 압축했다.
...
> v2.0 신규/보강은 A-7·A-15·A-16·A-18·A-19·E-2·E-7·F-4 **8건 (A-17 hold)**. 학술 인용 전문은 `references/scholarship.md`(이 벤더링본에는 미포함, 원본 저장소 참조). post-editese 3축 metric은 본 룰북 미반영(metric only 트랙). A-17 무정물·추상명사 '-들'은 학술 anchor(전영철 2007·곽은주·진실로 2011) 강하나 외부 회차(2026-05-07 위키 6편)에서 양성 0건 — NMT 원본 출력 회차 후 v2.1에서 동일 ID로 재평가.
