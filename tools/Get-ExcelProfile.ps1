...
.DESCRIPTION
  Opens every Excel file in the target folder READ-ONLY and extracts
  structure info into a single JSON file:
    - sheet list / visibility / used range size
    - top sample rows (default 8 rows x up to 40 cols) + last 2 rows
    - distinct formula patterns (up to 15)
    - per-column number formats
    - tables (ListObjects), pivot tables, defined names
    - external connections, Power Query formulas, external links
...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\Get-ExcelProfile.ps1 -Path "D:\TAM" -Recurse

.EXAMPLE
  # Mask confidential numbers as #NUM (integers 1900-2100 kept as years)
  powershell -ExecutionPolicy Bypass -File .\Get-ExcelProfile.ps1 -Path "D:\TAM" -MaskNumbers -OutFile tam_profile.json
#>
param(
